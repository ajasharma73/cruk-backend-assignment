...
jest.mock('aws-sdk', () => {
  const mSNS = {
    publish: jest.fn().mockImplementation(({ Message, MessageGroupId }, cb) => {
      if (DONATIONS_USER[MessageGroupId]) {
        expect(Message).toBe(
          util.format(DONATION_THANK_NOTE, DONATIONS_USER[MessageGroupId])
        );
        cb(null, {});
      } else {
